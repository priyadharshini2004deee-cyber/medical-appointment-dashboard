...
if not DATA_PATH.exists():
    alt_path = BASE_DIR / "Medical_appointment_data(1).csv"
    if alt_path.exists():
        DATA_PATH = alt_path
...
# =========================================================
@st.cache_data(show_spinner="Training optimized no-show model on full dataset...")
def train_classification_bundle(dataframe):
    """
    Memory-safe improved backend model:
    - Trains on FULL dataset, not sidebar filtered rows
    - Avoids 17GB memory error by limiting high-cardinality categories
    - Uses class weights instead of SMOTE
    - Uses CatBoost if installed; otherwise RandomForest fallback
    - Uses threshold tuning to improve F1 for No-Show class
    """
    ...
    if "appointment_date" in model_df.columns:
        model_df["appointment_date"] = pd.to_datetime(model_df["appointment_date"], errors="coerce")
        model_df["weekday_num"] = model_df["appointment_date"].dt.weekday.fillna(0).astype(int)
        model_df["is_weekend"] = np.where(model_df["weekday_num"] >= 5, 1, 0)
        model_df["month"] = model_df["appointment_date"].dt.month.fillna(0).astype(int)
        model_df["day"] = model_df["appointment_date"].dt.day.fillna(0).astype(int)
        model_df["year"] = model_df["appointment_date"].dt.year.fillna(0).astype(int)
    else:
        model_df["weekday_num"] = 0
        model_df["is_weekend"] = 0
        model_df["month"] = 0
        model_df["day"] = 0
        model_df["year"] = 0
    ...
    model_df["chronic_score"] = (
        model_df["hipertension"]
        + model_df["diabetes"]
        + model_df["alcoholism"]
        + model_df["handcap"]
    )

    for col in ["average_rain_day", "max_temp_day"]:
        if col not in model_df.columns:
            model_df[col] = 0
        model_df[col] = pd.to_numeric(model_df[col], errors="coerce").fillna(0)

    model_df["weather_risk"] = (
        (model_df["average_rain_day"] > 5).astype(int)
        + (model_df["max_temp_day"] > 35).astype(int)
    )
    ...
    num_cols = [
        ...
        "month", "day", "year", "weekday_num", "is_weekend",
        "chronic_score", "weather_risk"
    ]

    cat_cols = [
        "gender", "appointment_shift", "weekday",
        "rain_intensity", "heat_intensity",
        "specialty", "place", "disability"
    ]

    num_cols = [c for c in num_cols if c in model_df.columns]
    cat_cols = [c for c in cat_cols if c in model_df.columns]

    X_num = model_df[num_cols].copy() if num_cols else pd.DataFrame(index=model_df.index)
    for col in X_num.columns:
        X_num[col] = pd.to_numeric(X_num[col], errors="coerce")
    X_num = X_num.fillna(X_num.median(numeric_only=True)).fillna(0)

    if cat_cols:
        X_cat_base = model_df[cat_cols].copy()

        for col in X_cat_base.columns:
            X_cat_base[col] = X_cat_base[col].fillna("Unknown").astype(str)
            top_values = X_cat_base[col].value_counts().head(25).index
            X_cat_base[col] = np.where(
                X_cat_base[col].isin(top_values),
                X_cat_base[col],
                "Other"
            )
    ...
    if CATBOOST_AVAILABLE:
        ...
        model_name = "CatBoost + Class Weights + Threshold Tuning"
    else:
        ...
        clf.fit(X_train, y_train)
        model_name = "RandomForest + Class Weight + Threshold Tuning"
...
# =========================================================
st.markdown(
    '<div class="main-title">🏥 Medical Appointment Advanced Dashboard</div>',
    unsafe_allow_html=True
)
...
selected_gender = safe_multiselect(
    "Gender",
    gender_options,
    gender_options,
    "gender_filter"
...
use_all_places = st.sidebar.checkbox(
    "Use All Places / Cities",
    value=True
)
...
if use_all_places:
    ...
    selected_place = safe_multiselect(
        "Place",
        place_options,
        place_options,
        "place_filter"
...
selected_weekday = safe_multiselect(
    "Weekday",
    weekday_options,
    weekday_options,
    "weekday_filter"
...
selected_specialty = safe_multiselect(
    "Specialty",
    specialty_options,
    specialty_options,
    "specialty_filter"
...
selected_shift = safe_multiselect(
    "Appointment Shift",
    shift_options,
    shift_options,
    "shift_filter"
...
selected_status = safe_multiselect(
    "Attendance Status",
    status_options,
    status_options,
    "status_filter"
...
if selected_dates and isinstance(selected_dates, (list, tuple)) and len(selected_dates) == 2:
    ...
    if "appointment_date" in filtered.columns:
        filtered = filtered[
            filtered["appointment_date"].between(start_date, end_date)
        ]
...
noshow_rate = (
    noshow_count / total_appointments * 100
    if total_appointments > 0
    else 0
)

avg_age = (
    float(filtered["age"].mean())
    if "age" in filtered.columns and filtered["age"].notna().any()
    else 0.0
)

top_specialty = (
    filtered["specialty"].mode().iloc[0]
    if "specialty" in filtered.columns and filtered["specialty"].notna().any()
    else "N/A"
)

bundle = train_classification_bundle(df)
...
if selected_page == "Overview":
    ...
    m3.metric(
        "Unique Patients",
        f"{unique_patients_display:,}" if unique_patients_display is not None else "N/A"
    )
    ...
    st.markdown(
        f'<div class="info-box">{patient_note}</div>',
        unsafe_allow_html=True
    )

    st.markdown(
        f'<div class="good-box">Full dataset loaded: <b>{len(df):,}</b> records. Current filters showing: <b>{len(filtered):,}</b> records. Place filter is OFF by default.</div>',
        unsafe_allow_html=True
    )

    if total_unique_patients_full is not None:
        st.markdown(
            f'<div class="info-box">Total unique patients in full dataset: <b>{total_unique_patients_full:,}</b></div>',
            unsafe_allow_html=True
        )
    ...
    with c4:
        if "appointment_date" in filtered.columns and filtered["appointment_date"].notna().any():
            daily = (
                filtered.groupby(filtered["appointment_date"].dt.date)
                .size()
    ...
    patient_list = sorted(
        filtered["patient_id_final"].astype(str).unique().tolist()
    )
    ...
    patient_df = filtered[
        filtered["patient_id_final"].astype(str) == selected_patient
    ].copy()
    ...
    c2.metric(
        "Patient Attended",
        int((patient_df["no_show_label"] == "Attended").sum())
    )
    c3.metric(
        "Patient No-Show",
        int((patient_df["no_show_label"] == "No-Show").sum())
    )
    ...
    if "appointment_date" in patient_df.columns:
        st.dataframe(
            patient_df[show_cols].sort_values(
                by="appointment_date",
                ascending=False
            ),
    ...
    with c1:
        if "rain_intensity" in filtered.columns:
            rain_df = (
                filtered.groupby(["rain_intensity", "no_show_label"])
                .size()
            ...
            fig = px.bar(
                ...
                color="no_show_label",
                barmode="group",
                title="Rain Intensity vs Attendance"
            )
    ...
    with c2:
        if "heat_intensity" in filtered.columns:
            heat_df = (
                filtered.groupby(["heat_intensity", "no_show_label"])
                .size()
                .reset_index(name="Count")
            )
            ...
            fig = px.bar(
                ...
                color="no_show_label",
                barmode="group",
                title="Heat Intensity vs Attendance"
            )
    ...
    if weather_cols:
        summary = (
            filtered.groupby("no_show_label")[weather_cols]
            .mean()
            .reset_index()
        )
    ...
    fig = px.bar(
        ...
        y="Specialty",
        orientation="h",
        title="Top Specialties by Appointments"
    )
    ...
    if "specialty" in filtered.columns:
        ...
        summary_df["no_show_rate"] = np.where(
            summary_df["appointments"] > 0,
            summary_df["no_show_count"] / summary_df["appointments"] * 100,
            0
        )
    ...
    fig = px.bar(
        ...
        y="Place",
        orientation="h",
        title="Top Places by Appointments"
    )
    ...
    place_summary["no_show_rate"] = np.where(
        place_summary["appointments"] > 0,
        place_summary["no_show_count"] / place_summary["appointments"] * 100,
        0
    )

    st.dataframe(
        place_summary.sort_values(
            "appointments",
            ascending=False
        ),
        use_container_width=True
    ...
    if bundle is None:
        ...
        input_gender = c2.selectbox(
            "Gender",
            gender_options if gender_options else ["F", "M"]
        )

        input_sms = c3.selectbox(
            "SMS Received",
            [0, 1],
            index=0
        )

        input_shift = c4.selectbox(
            "Appointment Shift",
            shift_options if shift_options else ["Morning", "Afternoon"]
        )

        c5, c6, c7, c8 = st.columns(4)

        input_temp = c5.number_input(
            "Average Temperature",
            value=30.0
        )

        input_rain = c6.number_input(
            "Average Rain",
            value=0.0
        )

        input_rainy_before = c7.selectbox(
            "Rainy Day Before",
            [0, 1],
            index=0
        )

        input_storm_before = c8.selectbox(
            "Storm Day Before",
            [0, 1],
            index=0
        )
        ...
        input_weekday = c9.selectbox(
            "Weekday",
            weekday_options if weekday_options else ["Monday"]
        )

        input_heat = c10.selectbox(
            "Heat Intensity",
            sorted(df["heat_intensity"].dropna().unique().tolist())
            if "heat_intensity" in df.columns
            else ["Unknown"]
        )

        input_rain_intensity = c11.selectbox(
            "Rain Intensity",
            sorted(df["rain_intensity"].dropna().unique().tolist())
            if "rain_intensity" in df.columns
            else ["Unknown"]
        )

        input_time = c12.number_input(
            "Appointment Time",
            value=10.0
        )
        ...
        if st.button("Predict No-Show"):
            ...
            prob = float(
                bundle["model"].predict_proba(pred_df)[0][1]
            )
            ...
            st.caption(
                f"Model used: {bundle.get('model_name', 'ML model')} | Decision threshold: {threshold:.3f}"
            )

            if pred == 1:
                st.error(
                    f"Prediction: Likely No-Show ({prob_pct:.2f}% risk probability)"
                )
            else:
                st.success(
                    f"Prediction: Likely Attend ({100 - prob_pct:.2f}% attend confidence)"
                )
    ...
    forecast_source = st.radio(
        "Forecast Source",
        ["Use Full Data", "Use Filtered Data"],
        horizontal=True
    )
    ...
    if (
        "appointment_date" not in forecast_base.columns
        or forecast_base["appointment_date"].dropna().empty
    ):
        ...
        manual_start_date = fc1.date_input(
            "Forecast Start Date",
            value=(last_date + pd.Timedelta(days=1)).date()
        )

        forecast_days = fc2.slider(
            "Select Future Days",
            2,
            7,
            3
        ...
        if forecast_df.empty:
            st.warning(
                "Not enough data to build forecast. Use broader filters or full data."
            )
    ...
    if (
        "appointment_date" not in filtered.columns
        or filtered["appointment_date"].dropna().empty
    ):
        ...
        fig2.update_layout(
            title="Daily Appointments with 7-Day Moving Average",
            xaxis_title="Date",
    ...
    if bundle is None:
        ...
        c3.metric("Train Rows Used", f"{bundle['train_after']:,}")
        c4.metric("Test Rows", f"{bundle['test_rows']:,}")
        c5.metric("Features Used", f"{len(bundle['features'])}")

        if bundle["original_rows"] < 200:
            st.markdown(
                '<div class="weak-box">Model is weak because filtered training data is too small. Use broader filters for better results.</div>',
                unsafe_allow_html=True
            )
        ...
        st.markdown(
            f'<div class="info-box">Model: <b>{bundle.get("model_name", "ML Model")}</b> | Best Threshold: <b>{bundle.get("best_threshold", 0.5):.3f}</b> | Training uses full dataset, not sidebar filtered rows.</div>',
            unsafe_allow_html=True
        )

        if bundle["f1"] >= 0.75 and bundle["roc"] >= 0.80:
            st.markdown(
                '<div class="good-box">Model is GOOD.</div>',
                unsafe_allow_html=True
            )

        elif bundle["f1"] >= 0.60 and bundle["roc"] >= 0.70:
            st.markdown(
                '<div class="avg-box">Model is AVERAGE.</div>',
                unsafe_allow_html=True
            )

        else:
            st.markdown(
                '<div class="weak-box">Model is WEAK. Needs improvement.</div>',
                unsafe_allow_html=True
            )
    ...
    if patient_alerts_df.empty:
        ...
        high_alert = patient_alerts_df[
            patient_alerts_df["alert_level"] == "High Alert"
        ]

        medium_alert = patient_alerts_df[
            patient_alerts_df["alert_level"] == "Medium Alert"
        ]
        ...
        c3.metric(
            ...
            patient_alerts_df["patient_id_final"].nunique()
        )

        st.markdown(
            '<div class="alert-box">Patients are flagged based on repeated no-show behavior and risky no-show pattern.</div>',
            unsafe_allow_html=True
        )
    ...
    if (
        ...
        date_summary = filtered.groupby(
            filtered["appointment_date"].dt.date
        ).agg(
            ...
            avg_temp=(
                "average_temp_day",
                "mean"
            ) if "average_temp_day" in filtered.columns else (
                "appointment_date",
                "size"
            ),
            avg_rain=(
                "average_rain_day",
                "mean"
            ) if "average_rain_day" in filtered.columns else (
                "appointment_date",
                "size"
            )
        ).reset_index()
        ...
        date_summary["No-Show Rate"] = np.where(
            ...
            date_summary["No-Show"]
            / date_summary["Total Appointments"]
            * 100,
            0
        )
    ...
    st.caption(
        f"Showing {len(filtered):,} filtered rows out of {len(df):,} total rows."
    )
